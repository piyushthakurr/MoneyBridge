...
import { limit, SMALLEST_UNIT } from "../../../Constants/constant";
//import "./Users.css";
import "../../../App.css";

import {
  approveDisapproveMerchantFn,
  getMerchantlistFn,
} from "../../../Redux/Actions/MerchantCommission/merchant.action";
import { applypermissionRequestFn } from "../../../Redux/Actions/DepositeTrans/deposite.action";
...
function MerchantList() {
  ...
  const getMerchantlist = () => {
    ...
    dispatch(getMerchantlistFn(searchedData)).then((res) => {
  ...
  const approveOrDisapprove = () => {
    ...
    dispatch(
      approveDisapproveMerchantFn(
        approveRequestHandle?.users_id,
        actiontype == "approve" ? 1 : 2
      )
    ).then(
      (res) => {
  ...
  return (
    <div>
      <Container fluid className="main-content-container px-4">
        {/* Page Header */}
        <PageTitle title="Merchant List" subtitle="" />
        <Card className="mb-4">
          <Card.Body>
            <Row>
              <Col lg={4} md={12}>
            ...
            <>
              <Modal show={updateModal} onHide={() => setUpdateModal(false)}>
                <Modal.Header closeButton>
                  <Modal.Title>
                    Do you want to {actiontype} Merchant?
                  </Modal.Title>
                </Modal.Header>
