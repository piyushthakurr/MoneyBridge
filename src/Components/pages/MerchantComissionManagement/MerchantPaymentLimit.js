...
import { updateMerchantPaymentLimitFn } from "../../../Redux/Actions/MerchantCommission/merchant.action";

export default function MerchantPaymentLimit() {
  ...
  const openModalUpdate = (e) => {
    setWithdrawLimit(e.merchant_payment);
    setSid(e.sid);
    setUpdateModal(true);
  };
  ...
  const updateWithdrawLimit = () => {
    ...
    dispatch(updateMerchantPaymentLimitFn(sid, data)).then((val) => {
      fetchWithdrawLimit();
  ...
  return (
    <div>
      <Container fluid className="main-content-container px-4">
        <PageTitle title="Payment Limit" subtitle="" />
        <Card>
                ...
                <li>
                  {data.is_active != 0 && (
                    <div>
                      <p>
                        {" "}
                        Payment Limit {index === 0 ? "Without" : "With"}{" "}
                        Complete Level {index > 0 ? data?.type - 1 : 1}{" "}
                        (Monthly): {data?.merchant_payment} USD{" "}
                      </p>{" "}
