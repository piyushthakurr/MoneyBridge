...
function WithdrawTransactionFiat() {
  ...
  const getWithdrawTransactions = () => {
    ...
    dispatch(withdrawFiatRequestFn(filterData?.coin || "usd", page, data)).then(
      (val) => {
        let newArr = val.data;
  ...
  const calculateTotal = (amt, fee) => {
    console.log("asasasa", amt, fee);

    return parseFloat(amt) - parseFloat(fee);
  };
  const calcFee = (agentFee, fee, Exfee) => {
    return (parseFloat(fee) + parseFloat(Exfee) + parseFloat(agentFee)).toFixed(
      2
    );
  };
                  ...
                  {allWithdrawTrans.map((withdarwData, index) => {
                    return (
                      <tr>
                        ...
                        <td className="pl-3 text-nowrap">
                          {calcFee(
                            withdarwData?.payment_mode != "ACH_BANK" &&
                              withdarwData?.payment_mode != "VISA_CARD"
                              ? withdarwData?.payment_fee / SMALLEST_UNIT
                              : withdarwData?.fee / SMALLEST_UNIT,
                            withdarwData?.extra_fee / SMALLEST_UNIT,
                            withdarwData?.agent_fee / SMALLEST_UNIT || 0
                          )}
                        </td>
                        <td className="pl-3 text-nowrap">
                          {calculateTotal(
                        ...
                        <td className="pl-3 text-nowrap">
                          {withdarwData.status == 1 ? (
                            <p style={{ color: "brown" }}>UNCONFIRMED</p>
...
export default WithdrawTransactionFiat;
